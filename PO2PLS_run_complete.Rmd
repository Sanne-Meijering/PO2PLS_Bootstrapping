...
best_compML <- matrix(rep(0, 22*3), nrow = 22, ncol = 3, dimnames = dim_names)
likeli_array <- list()
for(i in 20:22){
  ...
  res <- max_logl_PO2PLS(Xtrain, ytrain, 7, 6, 6)
  best_compML[i,] <- res$maximum[1,]
  likeli_array[[i]] <- res$likelihood_array
}
save(best_compML, "best_compML", file="Components.Rdata")
save(likeli_array, "likeli_array", file="Components_array.Rdata")
```
...
for(i in 1:22){
  ...
  output_file <- paste("PO2PLS_ML_chr", i, ".Rdata", sep="")
